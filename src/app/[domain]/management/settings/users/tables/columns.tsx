...
export const columns = ({
  ...
  {
    accessorFn: (row) => {
      if (row.member?.firstName && row.member?.lastName) {
    ...
    cell: ({ row }) => {
      ...
      const displayName =
        firstName && lastName ? `${firstName} ${lastName}` : "No member";
  ...
  },
  {
    ...
    cell: ({ row }) => (
      <div className="truncate max-w-[200px]">{row.original.user?.email}</div>
    ),
    enableSorting: true,
    enableHiding: true,
    minSize: 200,

...
import { useUpdateMemberTenantUserId } from "@/entities/member/Performer.actions.client";
...
export default function ManageConnectionsForm({
  ...
  const updateMemberTenantUserId = useUpdateMemberTenantUserId(tenantId);
  ...
  const form = useForm<ManageConnectionsFormData>({
    ...
    defaultValues: {
      userId: performer.tenantUser?.user?.id || undefined,
  ...
  const onSubmit = async (data: ManageConnectionsFormData) => {
    try {
      ...
      const userIdChanged = data.userId !== performer.tenantUser?.user?.id;
      if (userIdChanged) {
        promises.push(
      ...
      <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
        ...
        <div className="flex justify-end space-x-2">
          ...
          <Button
            type="submit"
            disabled={
              updateConnections.isPending || updateMemberTenantUserId.isPending
            }
          >
            {updateConnections.isPending || updateMemberTenantUserId.isPending
              ? "Saving..."
